...
const EditEmploye = ({ employee, onBack, onUpdate }) => {
    ...
    const [formData, setFormData] = useState({
        ...
        iqama_no: employee?.iqama_no || employee?.iqama || '',
        muqeem_no: employee?.muqeem_no || '',
        iqama_expiry: employee?.iqama_expiry || '',
        passport_no: employee?.passport_no || '',
        passport_expiry: employee?.passport_expiry || '',
        gosi_no: employee?.gosi_no || '',
        address1: employee?.address1 || employee?.address || '',
        address2: employee?.address2 || '',
        joining_date: employee?.joiningDate || employee?.joining_date || '',
        ...
        <div className={`p-8 rounded-xl shadow-sm border h-full overflow-y-auto scrollbar-thin
            ...
            <div className="flex items-center justify-between mb-8 border-b pb-4 border-gray-200 dark:border-gray-700">
                <div className="flex items-center gap-4">
                    <button onClick={onBack} className={`p-2 rounded-full hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors ${darkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                        <ArrowLeft size={24} />
                    </button>
                    <div>
                ...
                <div className={`px-4 py-1.5 rounded-full text-sm font-medium ${darkMode ? 'bg-blue-900/30 text-blue-400' : 'bg-blue-50 text-blue-700'}`}>
                    {employee?.id || 'ID: N/A'}
                </div>
            </div>
...
// ⬇️ MOVED OUTSIDE to prevent re-renders losing focus
const FloatingInput = ({ label, id, value, onChange, type = "text", required = false, darkMode }) => (
